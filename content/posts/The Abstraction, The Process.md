...
OS에 의해서 여러 프로세스가 실행될 수 있습니다. 여러 프로세스들을 관리하기 위해 Process는 State를 가질 수 있습니다.
...
이 코드에 대한 동작을 그림으로 다시 살펴보겠습니다. 컴파일된 코드를 두 번 실행하여 두개의 프로세스가 실행되고 있는 상태라 가정해봅시다.
...
물리적 메모리에서는 페이지 단위로 데이터들이 나뉘어 있지만 추상화된 공간인 프로세스 안에서는 정리된 모습으로 표현가능합니다. 내용을 보면 같은 프로그램 코드로 두개의 프로세스가 생성이 되었습니다. 하드디스크의 프로그램 소스코드가 메모리의 코드 영역에 배치 되어있다는 것을 표현하기 위해 녹색으로 표시했습니다. 
